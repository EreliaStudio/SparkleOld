...
namespace spk
{
	...
	void Mesh::addVertex(const size_t& p_pointIndex, const size_t& p_uvIndex, const size_t& p_normalIndex)
	{
		if (_points.size() <= p_pointIndex)
			spk::throwException(L"Invalid point index : requested point [" + std::to_wstring(p_pointIndex) + L"] over [" + spk::to_wstring(_points.size()) + L"] stored points");
		if (_uvs.size() <= p_uvIndex)
			spk::throwException(L"Invalid uv index : requested uv [" + std::to_wstring(p_uvIndex) + L"] over [" + spk::to_wstring(_points.size()) + L"] stored uvs");
		if (_normals.size() <= p_normalIndex)
			spk::throwException(L"Invalid normal index : requested normal [" + std::to_wstring(p_normalIndex) + L"] over [" + spk::to_wstring(_normals.size()) + L"] stored normals");
	...
	void Mesh::addFace(const unsigned int& p_vertexIndexA, const unsigned int& p_vertexIndexB, const unsigned int& p_vertexIndexC)
	{
		if (_vertices.size() <= p_vertexIndexA)
			spk::throwException(L"Invalid vertex index : requested vertex [" + std::to_wstring(p_vertexIndexA) + L"] over [" + spk::to_wstring(_vertices.size()) + L"] stored vertices");
		
		if (_vertices.size() <= p_vertexIndexB)
			spk::throwException(L"Invalid vertex index : requested vertex [" + std::to_wstring(p_vertexIndexB) + L"] over [" + spk::to_wstring(_vertices.size()) + L"] stored vertices");
		
		if (_vertices.size() <= p_vertexIndexC)
			spk::throwException(L"Invalid vertex index : requested vertex [" + std::to_wstring(p_vertexIndexC) + L"] over [" + spk::to_wstring(_vertices.size()) + L"] stored vertices");
	...
	void Mesh::addFace(const unsigned int& p_vertexIndexA, const unsigned int& p_vertexIndexB, const unsigned int& p_vertexIndexC, const unsigned int& p_vertexIndexD)
	{
		...
		if (_vertices.size() <= p_vertexIndexD)
			spk::throwException(L"Invalid vertex index : requested vertex [" + std::to_wstring(p_vertexIndexD) + L"] over [" + spk::to_wstring(_vertices.size()) + L"] stored vertices");
