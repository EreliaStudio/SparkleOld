...
namespace spk
{
	...
	GLenum Pipeline::Buffer::_convertAttributeTypeToGLenum(const spk::ShaderLayout::Data::Type &p_input)
	{
		...
		case (spk::ShaderLayout::Data::Type::Float):
			return (GL_FLOAT);
		case (spk::ShaderLayout::Data::Type::Structure):
			return (GL_UNSIGNED_BYTE);
		}
	...
	Pipeline::Buffer::Buffer(const Pipeline::Buffer::Mode& p_mode, const ShaderLayout::StorageBufferLayout& p_configuration)
	{
		_mode = _convertModeToGLenum(p_mode);
		...
		for (const auto& field : p_configuration.fields())
		{
			...
			glVertexAttribPointer(
				field.location,
				field.data.format,
				_convertAttributeTypeToGLenum(field.data.type),
				GL_FALSE,
